...
**G. Cross-catalogue navigation — FAB + standalone bar (rewritten 2026-08-12, see §1l — supersedes the original 2026-08-08 chip-link version described in older drafts of this item)**
...
- [ ] In `generateRakhiIndexPage`/`generateRakhiCollectionPage` only, remove the `<div class="rakhi-shop-all-bar">...</div>` block (the standalone "Shop All Products" button below the chip strip) — this one has no equivalent on the regular-shop side, nothing to remove there.
- [ ] In `shop/shop.css`, remove `.cross-shop-fab`/`.cross-shop-fab--rakhi` (including the `@media (max-width: 600px)` override) and `.rakhi-shop-all-bar`/`.rakhi-shop-all-link`.
- [ ] Rebuild and confirm no floating button or standalone bar remains on either `/shop/` or `/shop/rakhi/` (and their sub-pages). The collection-chip strip itself (`collectionNavHtml()`) is unaffected either way — it no longer has any cross-link chip mechanism to worry about (that was removed in the 2026-08-12 rewrite, see §1l), so nothing to check there.
