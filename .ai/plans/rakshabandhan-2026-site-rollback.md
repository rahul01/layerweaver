...
**Updated 2026-08-08:** see new §1c (discount code names dropped from copy, informational only), §1d (logo ribbon badge — expanded from homepage-only to sitewide same day, touching 16+ files — see rewritten §3 item H), §1e (cross-catalogue chip links added between regular shop and Rakhi pages — see §3 item G, the one change here that touches code shared with the non-Rakhi shop and needs a user judgement call before reverting), §1f ("All" chip renamed to "All Rakhi Gifts", Rakhi-only, low risk), §1g (festive page-level theming — cream background, amber accent, ribbon badge, SVG thread motif — added to Rakhi pages, entirely additive/Rakhi-scoped), §1h (festive theming expanded further same day — background pattern, bigger banner, grid divider, product-card accents), and §1i (thread motif redesigned from 2 loops to an 8-petal mandala, same day — see §3 item I, now covering §1g/§1h/§1i together).
**Updated 2026-08-09:** see new §1j (announcement bar restyled to Rakhi amber + mobile auto-scroll marquee added — genuinely sitewide, NOT gated behind `body.rakhi-theme` like §1g-§1i, see updated **item A** rather than item I). Also fixed a gap discovered along the way: 11 hand-authored pages besides `index.html` still had stale `(code RAKHI05)`/`(code RAKHI10)` text from before §1c's fix — corrected as part of the same edit.
**Updated 2026-08-12:** see new §1k — every hero carousel slide (added by §1b's original hero takeover) had been 404ing since 2026-08-07 due to a stale link-path template, only caught and fixed now. **Item F's revert instructions above have been corrected** to include the link-path fix, not just the call-site argument swap — read §1k before executing item F. Also see new §1l — the cross-catalogue chip link (§1e/§1f) was fully removed and replaced with a FAB + standalone bar; **item G has been rewritten, not extended** — read §1l before executing item G. And §1m — the standalone bar from §1l was itself removed later the same day, FAB only remains; item G updated again to match.
...
### 1h. Festive theming expanded to full page (2026-08-08, same day — item I below extended, not a new item)

Same day as §1g, the festive treatment was expanded beyond just the banner (per owner feedback that the rest of the page still looked plain): a tiled background pattern on `body.rakhi-theme`, a bigger/bordered banner with a second corner motif, a new `rakhiGridDividerHtml()` divider between the banner and grid, and an amber corner-ribbon + border on `body.rakhi-theme .shop-product-card` (CSS-only, `productCardHtml()` itself untouched). Still entirely additive/Rakhi-scoped — same low-risk profile as §1g, same **item I** checklist below now covers both.

### 1i. Thread motif redesigned as an 8-petal mandala (2026-08-08, same day — item I below extended again, still not a new item)
...
### 1j. Announcement bar restyled + mobile marquee added — genuinely sitewide, NOT gated behind body.rakhi-theme (2026-08-09 — see updated item A above, not item I)

**Read this carefully if you've been following the §1g-§1i pattern of "everything Rakhi-themed lives behind `body.rakhi-theme`/`.rakhi-*` selectors and is covered by item I."** The announcement bar breaks that pattern: unlike the banner/divider/product-card theming (§1g-§1i), the bar itself is NOT Rakhi-page-scoped — it was already sitewide *before* any Rakhi campaign work started (it's the original masterplan Step A component, present on literally every page including regular non-Rakhi shop pages). So when it was restyled to the Rakhi amber palette and given a mobile auto-scroll marquee (masterplan §0 item 16), that change also landed sitewide by necessity — there's no "Rakhi pages only" version of this bar to gate it behind.

**Practical effect on rollback:** removing the announcement bar entirely (item A, which this rollback plan already does regardless of styling) automatically removes this too — no separate action needed if item A executes correctly. But if a future session ever wants to revert *just* the Rakhi styling while keeping the announcement bar itself (e.g. reverting to the old purple color but keeping the bar), that's a distinction this document has never had to make before and item A's instructions (which assume the whole bar goes away) don't cover it — flag that as a new judgement call for the user rather than assuming from this document alone. Item A above has been updated to mention the 11 additional hand-authored pages this touches and the new `.announcement-bar-track`/`@keyframes announcement-marquee` CSS.

### 1k. Hero carousel 404 bug fixed — corrects item F's link-revert instructions (2026-08-12)

Every one of the 8 homepage hero carousel slides (added by the original 2026-08-07 hero takeover, §1b) 404'd from launch until this was caught and fixed on 2026-08-12 — `heroCarouselSlidesHtml()`'s slide link template still said `shop/collections/${collection.handle}/` (correct for the pre-takeover regular-collections carousel) even after the function started receiving `rakhiCollections` instead, whose pages actually live at `shop/rakhi/${collection.handle}/`. Fixed by correcting the hardcoded path (masterplan §0 item 17 has full detail). **Item F above has been updated** with an explicit warning: reverting item F means changing the link template back to `shop/collections/...` in the same edit as swapping the call-site argument, not just the argument alone — doing only the argument swap would reintroduce the identical bug in reverse (regular-collection handles hitting a now-wrong `shop/rakhi/` path). This is a good reminder that **link destinations, not just visual rendering, need verification** after any hero/carousel change — the original bug went unnoticed for 5 days despite this document's own verification checklists being followed.

### 1l. Cross-catalogue chip link replaced with FAB + standalone bar — item G rewritten, not extended (2026-08-12)

§1e/§1f's `crossLink` chip mechanism (the small "🪢 Rakhi Gifts"/"← Full Shop" chip inside the collection-filter strip) has been **completely removed and replaced**, not extended — owner feedback was that it was too easy to miss and "Full Shop" was the wrong label. `collectionNavHtml()`'s `crossLink` parameter is gone from its signature entirely (not just unused — actually deleted, along with the `.collection-nav-chip--cross`/`.collection-dropdown-item--cross` CSS). In its place: a persistent floating action button (`crossShopFabHtml()`) on both sides, plus a standalone "Shop All Products" button below the chip strip on Rakhi pages only. See masterplan §0 item 18 for full detail. **Item G above has been fully rewritten** to match — if you're looking at an old cached version of this document or working from memory of an earlier read, the chip-based instructions no longer apply; re-read item G before executing.

### 1m. Standalone "Shop All Products" bar removed same day, FAB kept (2026-08-12)

The `.rakhi-shop-all-bar` standalone button described in §1l/masterplan §0 item 18 was itself removed a short time later the same day, per explicit owner request — the FAB alone was judged sufficient once both were seen together. `generateRakhiIndexPage()`/`generateRakhiCollectionPage()` no longer render it, and `.rakhi-shop-all-bar`/`.rakhi-shop-all-link`/`.rakhi-shop-all-link:hover` are gone from `shop/shop.css`. **The FAB (`crossShopFabHtml()`) is untouched and still present on all 4 page types** — only the extra top-of-page bar was removed. **Item G above has been updated again** to reflect this (its title changed from "FAB + standalone bar" to "FAB only") — the bar-removal checklist line has been replaced with a note explaining there's nothing left to remove there.
...
- [ ] Remove the equivalent hand-authored markup from **`index.html` AND the 11 other hand-authored pages that carry their own independent copy** (added 2026-08-09 per masterplan §0 item 16 — easy to miss if only checking `index.html`): `404.html`, `workshop/`, `enroll/`, `enroll-adult/`, `privacy-policy/`, `faq/`, `gallery/`, `return-and-exchange-policy/`, `terms-of-service/`, `connect/`, `shipping-policy/`. Search `grep -rl "announcement-bar\"" --include="*.html" .` to find every copy rather than trusting this list, in case more pages were added since. (`services/3d-design/`, `services/on-demand/`, `shop/collage/` never had this markup — don't add it there as part of "completing" the removal.)
- [ ] Remove the `.announcement-bar`/`.announcement-bar-track` CSS rules and the `@keyframes announcement-marquee` animation from `shop/shop.css` and `styles.css` (the marquee/mobile-scroll behavior and Rakhi-amber color, both added 2026-08-09 per masterplan §0 item 16, are part of the same block being removed here — no separate cleanup step needed for those).
...
- [ ] In `scripts/build-shop.js`, change the `heroCarouselSlidesHtml(rakhiCollections)` call (~line 2101) back to `heroCarouselSlidesHtml(collections)` — restores hero slides sourced from regular (non-Rakhi) collections. **Critical, easy to miss:** `heroCarouselSlidesHtml()`'s slide link template was `shop/collections/${collection.handle}/` originally, but was corrected on 2026-08-12 (masterplan §0 item 17, a bug that shipped with the original 2026-08-07 takeover and went unnoticed for 5 days — every slide 404'd) to `shop/rakhi/${collection.handle}/`, matching the Rakhi collections it now receives. **If reverting this item, the link template must be changed back to `shop/collections/${collection.handle}/` in the same edit** — simply swapping the call-site argument back to `collections` without also fixing the hardcoded path will produce the exact same class of 404 bug in reverse (regular-collection handles hitting a `shop/rakhi/` path that doesn't exist for them). Verify with the same method used to catch this originally: after rebuilding, `curl` or click through every resulting hero slide URL and confirm HTTP 200 — don't just check that the carousel renders visually.
...
**G. Cross-catalogue navigation — FAB only (rewritten 2026-08-12, see §1l/§1m — supersedes the original 2026-08-08 chip-link version described in older drafts of this item)**

**Ask the user first** whether cross-catalogue navigation (in any form) should be removed entirely, or kept as evergreen discoverability even if the Rakhi catalogue itself stays live post-campaign (see §6 — this is explicitly not a call to make unilaterally). If keeping it, skip this item — none of it is Rakhi-*content*-specific in a way that requires removal just because the campaign ended.

If removing:
- [ ] In `scripts/build-shop.js`, remove `crossShopFabHtml()` (near `collectionNavHtml()`) and its 4 call sites — `${crossShopFabHtml(shopBase, true)}` in `generateShopIndex`/`generateCollectionPage`, `${crossShopFabHtml(shopBase, false)}` in `generateRakhiIndexPage`/`generateRakhiCollectionPage`.
- [ ] In `shop/shop.css`, remove `.cross-shop-fab`/`.cross-shop-fab--rakhi` (including the `@media (max-width: 600px)` override).
- [ ] Rebuild and confirm no floating button remains on either `/shop/` or `/shop/rakhi/` (and their sub-pages). The collection-chip strip itself (`collectionNavHtml()`) is unaffected either way — it no longer has any cross-link chip mechanism to worry about (that was removed in the 2026-08-12 rewrite, see §1l), so nothing to check there.
- [ ] **Note:** there is no standalone "Shop All Products" bar to remove — item 18's version of it (a `.rakhi-shop-all-bar` band below the Rakhi chip strip) was itself removed the same day per owner request (masterplan §0 item 19, §1m below); only the FAB survived past 2026-08-12. Don't go looking for bar-related markup/CSS that no longer exists.
...
**I. Festive Rakhi page theming (added per §1g/§1h, 2026-08-08 — additive/Rakhi-scoped, low risk)**
- [ ] In `scripts/build-shop.js`: remove `rakhiThreadMotifSvg()`, `rakhiFestiveBannerHtml()`, and `rakhiGridDividerHtml()` (three functions, near `collagebannerHtml()`). In `generateRakhiIndexPage()` and `generateRakhiCollectionPage()`: revert the `rakhiFestiveBannerHtml(...)` call back to plain `collagebannerHtml(...)`, remove the `${rakhiGridDividerHtml()}` call directly below each banner call, and remove the `rakhi-theme` class from each function's `<body>` tag.
- [ ] In `shop/shop.css`: remove the entire "Rakhi festive theme" block — `body.rakhi-theme` (including its tiled `background-image` data-URI), `.rakhi-banner-wrap` and `.rakhi-banner-wrap .collection-banner` (the amber border/box-shadow/height overrides), `.rakhi-special-badge`, `.rakhi-banner-motif` and `.rakhi-banner-motif--corner`, `.rakhi-thread-motif`, `.rakhi-grid-divider` and its children, the `@media` overrides for all of the above, the `body.rakhi-theme .collection-nav-chip.active`/`.collection-dropdown-item.active` amber overrides, and the `body.rakhi-theme .shop-product-card`/`:hover` product-card accent rules (masterplan §0 item 15: the card's `::before` corner-ribbon triangle was already removed on 2026-08-08 — don't expect to find it, only the `border` and `:hover` glow rules remain to revert here).
...
- [ ] Rebuild and confirm Rakhi pages look like plain shop pages again — white background (no tiled pattern), standard-height banner with no border/glow/corner-motif/badge, no divider between banner and grid, plain product cards (no corner ribbon, no amber border/hover glow), standard purple active-chip color. Regular shop pages should show zero diff throughout (this change never touched anything outside `body.rakhi-theme`/`.rakhi-*` selectors and the two Rakhi generator functions — `productCardHtml()` itself was never modified).
